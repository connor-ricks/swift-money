...
/// given ``Currency`` objects, trading the monetary values of one currency for monetary
/// values of another.
...
extension AsyncExchange {
    /// Attempts to convert the provided monetary value into the provided currency using
    /// the exchange's conversion rate.
    ...
    ///
    /// Each of these monetary values will then be summed in order to provide a monetary value in
    ...
    public func trade<C: Currency>(
        ...
        for aggregatedCurrency in aggregatedMoney.currencies {
            ...
            if aggregatedCurrency.isEqual(to: currency) {
                ...
                let rate = try await rate(
                    base: currency,
                    quote: aggregatedCurrency
                )
                money += amount / rate
            }
        }
